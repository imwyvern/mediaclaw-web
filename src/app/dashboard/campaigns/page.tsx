...
import { Plus, Target, Calendar, ArrowRight, Rocket, Loader2 } from "lucide-react";
      ...
      {loading && campaigns.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {campaigns.map((campaign) => (
            <Card key={campaign.id} className="flex flex-col cursor-pointer hover:border-primary/50 transition-colors group">
              <CardHeader className="pb-3">
              ...
              <CardContent className="flex-1 space-y-6 pb-3">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm border-t pt-4">
                  <div>
                    <div className="text-muted-foreground flex items-center mb-1 text-[10px] uppercase font-bold tracking-tight"><Calendar className="w-3.5 h-3.5 mr-1" /> Start Date</div>
                    <div className="font-medium">{campaign.startDate}</div>
                  </div>
                  <div>
                    <div className="text-muted-foreground flex items-center mb-1 text-[10px] uppercase font-bold tracking-tight"><Target className="w-3.5 h-3.5 mr-1" /> Platforms</div>
